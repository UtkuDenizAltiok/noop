...
metadata:
  ...
  modified: 2026-09-15T13:38:29.828Z
---
...
- Merged upstream: #2098 schema, #2099 app (with ryanbr's empty-session guard `fed714cb`), #2232 the
  ...
  that gate red).
- Follow-up on `lift-log-discard-and-edit` (one Save, discards as zeros fillable in Edit sets, add/remove in
  Edit sets, Kotlin twin in step): locally rebased to `0a55bc8c` on `c430ca0b`, not yet re-verified or pushed
  because Xcode 27's license is unaccepted. The fork holds verified `a7f2ad08`; the phone has build `775a1819`.
- Open question before the PR: `WhoopStore.deleteLiftSets` is a new Swift-only function — the parity ratchet
  may flag it (#2163: dispositions cannot settle `add-unpaired-function`).
