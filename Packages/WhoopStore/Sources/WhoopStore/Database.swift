...
extension WhoopStore {
    ...
    static func makeMigrator() -> DatabaseMigrator {
        ...
        // v40-lift-log: the in-app strength log — saved programs and the sessions run from them.
        //
        ...
        //   • ifNotExists so a fork that already carries these tables under another identifier
        //     converges instead of failing the migrator (the v38 idiom).
        //
        // Pinned in schema_oracle.json as `ios_only` with a stated reason: the Room twin is a follow-up,
        // not part of this change.
        migrator.registerMigration("v40-lift-log") { db in
            ...
            try db.create(index: "idx_liftExercise_natural", on: "liftExercise",
                          columns: ["deviceId", "name"], unique: true)
            ...
            try db.create(index: "idx_liftProgram_device_updatedAt",
                          on: "liftProgram", columns: ["deviceId", "updatedAt"])
            ...
            try db.create(index: "idx_liftSession_natural", on: "liftSession",
                          columns: ["deviceId", "startTs", "sport"], unique: true)
