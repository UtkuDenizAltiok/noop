...
extension WhoopStore {
    ...
    static func makeMigrator() -> DatabaseMigrator {
        ...
        // v42-lift-log: the in-app strength log — saved programs and the sessions run from them.
        //
        ...
        //   • EVERY create here is `ifNotExists` — tables AND indexes — so this migration is a
        //     no-op against a database that already carries the schema (the v38 idiom). That is
        //     load-bearing, not decorative: this feature was first developed as `v40-lift-log`, and
        //     upstream took v40/v41 for the skin-temp and IMU changes, so the identifier had to move
        //     to v42. GRDB keys applied migrations by identifier and silently ignores identifiers it
        //     does not know (`appliedMigrations` intersects with the registered set), so a device
        //     that ran the old `v40-lift-log` sees `v42-lift-log` as unapplied and runs it again.
        //     Idempotent statements are what makes that re-run harmless instead of a launch failure
        //     on "table liftExercise already exists". The two orphan rows the old identifiers left in
        //     `grdb_migrations` are inert.
        //
        // Pinned in schema_oracle.json as `ios_only` with a stated reason: the Room twin is a follow-up,
        // not part of this change.
        migrator.registerMigration("v42-lift-log") { db in
        ...
        // v43-lift-log-targets: two columns the first gym session showed were missing.
        //
        // `liftProgramItem.targetWeightKg` — a program line plans a WEIGHT, not just a rep range. The
        // v42 shape carried `targetRepsLow`/`targetRepsHigh`/`targetRpe` (a literal reading of one
        ...
        //
        // Both ALTERs are column-guarded for the same reason every create in `v42-lift-log` is
        // `ifNotExists`: this was developed as `v41-lift-log-targets` before upstream took v41, so a device
        // that already ran the old identifier will run this one again, and an unguarded
        // ADD COLUMN fails the migrator on "duplicate column name" and takes the app's launch with it.
        migrator.registerMigration("v43-lift-log-targets") { db in
            if try !db.columns(in: "liftProgramItem").contains(where: { $0.name == "targetWeightKg" }) {
                try db.alter(table: "liftProgramItem") { t in
                    t.add(column: "targetWeightKg", .double)   // kilograms, like every stored weight
                }
            }
            if try !db.columns(in: "liftSession").contains(where: { $0.name == "sessionRpe" }) {
