...
final class LiftLogStoreTests: XCTestCase {
    ...
    // MARK: - v40 migration (additive: four new tables + indexes, nothing dropped)

    func testV40CreatesLiftLogTables() async throws {
    ...
    func testV40ColumnsArePresent() async throws {
        let store = try await WhoopStore.inMemory()
        ...
        for c in ["id", "deviceId", "startTs", "endTs", "sport", "programId", "programName", "note"] {
    ...
    /// Additive: v40 must not drop any table that existed before it.
    func testV40IsAdditive() async throws {
        let store = try await WhoopStore.inMemory()
        let tables = try await store.tableNames()
        for t in ["device", "hrSample", "rrInterval", "event", "battery", "rawBatch",
            ...
            XCTAssertTrue(tables.contains(t), "v40 must not drop \(t)")
        }
    }
