...
final class LiftLogStoreTests: XCTestCase {
    ...
    // MARK: - v45 migration (additive: five new tables + indexes, nothing dropped)

    func testV45CreatesLiftLogTables() async throws {
    ...
    func testV45ColumnsArePresent() async throws {
        let store = try await WhoopStore.inMemory()
    ...
    func testV45CreatesIndexes() async throws {
        let store = try await WhoopStore.inMemory()
    ...
    /// Additive: v45 must not drop any table that existed before it.
    func testV45IsAdditive() async throws {
        let store = try await WhoopStore.inMemory()
        let tables = try await store.tableNames()
        for t in ["device", "hrSample", "rrInterval", "event", "battery", "rawBatch",
            ...
            XCTAssertTrue(tables.contains(t), "v45 must not drop \(t)")
        }
    }
